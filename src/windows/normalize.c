...
_windows_normalize(PyUnicodeObject *arg)
{
    ...
    if (drive.index == arg_size && drive.kind != WINDOWS_DRIVE_VOLUME && drive.kind != WINDOWS_DRIVE_DEVICE_NORMALIZED)
    {
    ...
    {
        if (drive.kind != WINDOWS_DRIVE_VOLUME && drive.kind != WINDOWS_DRIVE_DEVICE_NORMALIZED)
        {
            cow_write(&cow, '\\');
        }
